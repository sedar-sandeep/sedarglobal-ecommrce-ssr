...
const SeoHeader = ({ data, router }) => {
    // console.log(data?.SEO, 'test by daat');
    ...
    if (data && data.SEO && data.SEO !== 'null') {
        if (Array.isArray(data.SEO)) {
            seo = data.SEO[0] || {}; // [{}] hoga to {} aa jayega
        } else {
            seo = data.SEO; // direct object hoga to wahi assign hoga
        }
    }
    let meta_title = seo && seo.SEO_META_TITLE && seo.SEO_META_TITLE != 'null' ? seo.SEO_META_TITLE : 'Sedar';
    const isNoIndex = NOINDEX_LOCALES.includes(locale);
    //console.log(meta_title,' meta_title test by shahid');
    ...
    if (metaTitleInclude || metaDescInclude) {
        ...
        const mapObj = {
            '[CATEGORY_NAME]': categoryData && categoryData[0]?.content || '',
            '[COUNTRY]': country_iso,
        ...
        og_title = meta_title;
        og_desc = meta_desc;
        og_image = categoryData && categoryData[0]?.banner_image_path || '';
        og_image_width = 200;
        og_image_height = 200;
        // og_type = 'website';
