...
import { valanceType, addToCartFunScene } from '../Scene'
...
const ValanceOption = (props) => {
  ...
  const optionFun = (val) => {
    ...
    if (val.SPS_CODE === 'VAL02') {
      delete customize_state.steps.VALANCE_COLOR;
    }
  ...
  useEffect(() => {
    ...
    props.SUB_CHILD.map((data) => {
      ...
      if (edit_step.info_result && edit_step.info_result.VALANCE_OPTION && edit_step.info_result.VALANCE_OPTION.SOI_SPS_CODE == data.SPS_CODE) {
...
ValanceOption.propTypes = {};

ValanceOption.defaultProps = {};
