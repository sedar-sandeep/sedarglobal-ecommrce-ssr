...
import { controlType, addToCartFunScene } from '../Scene';
...
const ControlType = (props) => {
  ...
  useEffect(() => {
    ...
    props.SUB_CHILD.map((data) => {
      if (edit_step.info_result && edit_step.info_result.CONTROL_TYPE && edit_step.info_result.CONTROL_TYPE.SOI_SPS_CODE == data.SPS_CODE) {
  ...
  const optionFun = (val) => {
    ...
    if (val.SPS_CODE == 'CT01') {
      ...
      delete customize_state.steps.LENGTH_OF_WIRE;
    } else if (val.SPS_CODE == 'CT02') {
      delete customize_state.steps.OPERATING_SIDE;
    }
