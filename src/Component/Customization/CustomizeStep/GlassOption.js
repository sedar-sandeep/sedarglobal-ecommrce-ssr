...
import { glassOption,addToCartFunScene } from '../Scene';
...
const GlassOption = (props) => {
  ...
  const optionFun = (val) => {
    glassOption(val.SPS_CODE);
    setSelected(val.SPS_CODE);
    setStepArray(val);
    customizeDispatch(val);
    if (val.SPS_CODE == 'GL01') {
      delete customize_state.steps.GLASS_COLOR;
    }
  ...
  useEffect(() => {
    ...
    props.SUB_CHILD.map((data) => {
      ...
      if (edit_step.info_result && edit_step.info_result.GLASS_OPTION && edit_step.info_result.GLASS_OPTION.SOI_SPS_CODE == data.SPS_CODE) {
...
GlassOption.propTypes = {};

GlassOption.defaultProps = {};
