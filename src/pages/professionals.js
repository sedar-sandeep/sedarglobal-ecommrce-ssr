...
export async function getServerSideProps(context) {
    ...
    const response = await axiosInstance.get(`v2/homepage/first?content=professionals&locale=${locale}`).catch((error) => {
        console.error('Professionals API Error:', error);
        return { data: null };
    });

    const headerRes = await axiosInstance.get(`header/first?site=${siteId}&lang=${lang}&country=${country}&visitorId=${visitor}&userId=${user}&content=header&slug_url=header&cn_iso=${cn_iso}&detect_country=${detect_country}&page_name=professionals`).catch((error) => {
    ...
    return {
        props: {
            ...
            home_url: `v2/homepage/first?content=professionals&locale=${locale}`
