...
export async function getServerSideProps(context) {
    ...
    const response = await axiosInstance.get(`v2/homepage/first?content=contracts&locale=${locale}&slug_url=${slug_url}`);

    const headerRes = await axiosInstance.get(`header/first?site=${siteId}&lang=${lang}&country=${country}&visitorId=${visitor}&userId=${user}&content=header&slug_url=header&cn_iso=${cn_iso}&detect_country=${detect_country}&page_name=${slug_url}&seo_type=contracts`);
