...
export async function getServerSideProps(context) {
    ...
    let blog_url = `v2/homepage/first?content=blog&locale=${locale}&slug_url=${slug_url}`;
    const response = await axiosInstance.get(blog_url).catch((error) => {
        console.error('Blog API Error:', error);
        return { data: null };
    });

    const headerRes = await axiosInstance.get(`header/first?site=${siteId}&lang=${lang}&country=${country}&visitorId=${visitor}&userId=${user}&content=header&slug_url=header&cn_iso=${cn_iso}&detect_country=${detect_country}&page_name=blog`).catch((error) => {
