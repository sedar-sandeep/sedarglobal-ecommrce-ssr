...
export async function getServerSideProps(context) {
    ...
    // PageData
    const cartResponse = await axios.get(`${process.env.NEXT_PUBLIC_LOCAL_API_URL}api/cartPage.json`);

    const headerRes = await axiosInstance.get(`header/first?site=${process.env.NEXT_PUBLIC_SITE_ID}&lang=${lang}&country=${country}&visitorId=${visitor}&userId=${user}&content=header&slug_url=header&cn_iso=${cn_iso}&detect_country=${detect_country}&page_name=cartpage`);
    ...
    return {
        props: {
            ...
            headRes: `header/first?site=${process.env.NEXT_PUBLIC_SITE_ID}&lang=${lang}&country=${country}&visitorId=${visitor}&userId=${user}&content=header&slug_url=header&cn_iso=${cn_iso}&detect_country=${detect_country}&page_name=cartpage`
        },
    };

}

function CartSupportPage(props) {
    //const { cartResponse } = props;
    //console.log(props, 'carthead')
    return (
        <React.Fragment>
            <div>
                <Script
...
export default CartSupportPage;