...
export const getServerSideProps = wrapper.getServerSideProps(store => async (context) => {
    ...
    let header_url = `header/first?site=${siteID}&lang=${lang}&country=${country}&visitorId=${visitor}&userId=${user}&content=header&slug_url=header&cn_iso=${cn_iso}&detect_country=${detect_country}&page_name=motorized_guide`;
    let footer_url = `footer/first?site=${siteID}&lang=${lang}&country=${country}&content=footer&slug_url=footer&cn_iso=${cn_iso}`;
    let content_url = `content/first?site=${siteID}&lang=${lang}&country=${country}&content=motorized_guide&visitorId=${visitor}&userId=${user}&cn_iso=${cn_iso}&_limit=20&_page=0`;
    ...
    if (contentRes && contentRes?.data?.CSP) {
        if (locale == 'default') {
            return {
                redirect: {
                    destination: `/`,
