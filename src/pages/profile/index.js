...
export async function getServerSideProps(context) {
    ...
    const auth_token = context.req.cookies['AUTH_TOKEN'] || "";
    const siteId = process.env.NEXT_PUBLIC_SITE_ID;

    if (auth_token) {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_LOCAL_API_URL}api/Profile.json`);
...
function Profile(props) {
    ...
    useEffect(() => {
        if (!props?.user_state?.isLoggedIn) {
